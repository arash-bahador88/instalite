...
def likepost(request):
    ...
    like_post = likePost.objects.filter(post_id = post_id).first()
...
def comment(request):
    if request.method == "POST" :
        ...
        post = Post.objects.create.filter(id = post_id ).first()


        new_comment= PostComment.objects.filter(user=user, post_id=post_id, content=comment)
        new_comment.save()
        ...
        return render(request, "index.html", {"comments":post_comments})
